...
description: 在对话开始时主动分析用户请求，判断任务类型、评估提示词质量，并智能推荐应激活的技能。建议对所有请求先做分诊，以保障工作流最优。通过清晰度、具体度、完整度评估决定是否调用 prompt-optimizer；识别 UI 设计任务时推荐 ui-analyzer；识别 React 组件请求时推荐 react-component-generator。作为技能协调中枢使用。
